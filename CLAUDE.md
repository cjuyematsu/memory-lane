...
The notification gating pipeline (in `handleClusterEnter`): `enabled` → `isAreaNotifiable` (cluster's newest photo older than ~90 days **and** no recent media anywhere in a 150m neighborhood) → **routine-place suppression** (`lib/place-presence.ts`) → cooldown (`lib/notification-cooldown.ts`, 12h within 150m). The routine gate is the real home/work silencer: every enter logs a presence ping (`recordPresence`), and `isRoutineLocation` mutes a spot once you've been there on ≥3 distinct days inside a rolling 14-day, 150m window — so home/work/gym go quiet after ~2 days and *resurface* when you stop coming (move out / change jobs). It replaced an old tap-engagement scheme. The greeter (below) records presence too, so the signal works even with notifications off. Foreground notifications are suppressed at the system level (`setNotificationHandler` returns all-false) and shown as an in-app banner (`lib/foreground-banner.ts` → `components/notifications/memory-banner.tsx`) instead. The location cooldown is a `createLocationCooldown({file,windowMs,radiusM})` factory (`lib/location-cooldown.ts`); the geofence one is the 12h instance. **Those radii are no longer fixed**: `lib/relevance-radius.ts` (`relevanceRadiusFor` = ½ the distance to your k-th=2 nearest *other* cluster, clamped) makes the geofence trigger, the `isAreaNotifiable` suppression neighborhood, the cooldown radius, and the Near Me grid (`computeNearby`'s `radiusM`) all adapt to local photo-cluster density — tight in dense cities, wide at a spread-out beach. Floors keep dense areas at the old constants (the geofence floor is the ~120m iOS region-accuracy limit, so the OS trigger only widens; in-app checks can also tighten). `isInCooldown(lat,lng,now?,radiusOverride?)` takes a per-call radius.
